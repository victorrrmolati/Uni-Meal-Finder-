// ============================================================
//  src/pages/Checkout.tsx — Updated to navigate to orders
//  after placing an order so user can see and edit it.
// ============================================================
...
export default function Checkout({ onOrderPlaced }: CheckoutProps) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: '16px 20px', width: '100%', border: '0.5px solid #e2e8f0', marginTop: 20 }}>
          ...
          <div style={{ background: '#0f172a', borderRadius: 14, padding: '12px 20px', width: '100%', marginTop: 12, textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 16, textAlign: 'center' }}>
            You can edit or cancel this order from your orders page while it's still pending
          </p>
          {onOrderPlaced && (
            <button
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ background: 'white', borderRadius: 18, padding: '18px 16px', border: '0.5px solid #e2e8f0' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <MethodBtn active={deliveryMethod === 'delivery'} onClick={() => { setDeliveryMethod('delivery'); setPaymentMethod('on_delivery'); }}
              icon={<Truck size={22} color={deliveryMethod === 'delivery' ? '#16a34a' : '#94a3b8'} />} label="Delivery" sub="To your location" />
            <MethodBtn active={deliveryMethod === 'pickup'} onClick={() => { setDeliveryMethod('pickup'); setPaymentMethod('on_pickup'); }}
              icon={<ShoppingBag size={22} color={deliveryMethod === 'pickup' ? '#16a34a' : '#94a3b8'} />} label="Pickup" sub="Collect yourself" />
          </div>
        </div>
        ...
        {deliveryMethod === 'delivery' && (
          <div style={{ background: 'white', borderRadius: 18, padding: '18px 16px', border: '0.5px solid #e2e8f0' }}>
            ...
            <input value={address} onChange={e => setAddress(e.target.value)} placeholder="e.g. Dorm 3, Room 204"
              style={{ width: '100%', padding: '12px 14px', borderRadius: 12, boxSizing: 'border-box', border: '1.5px solid #e2e8f0', fontSize: 14, color: '#0f172a', background: '#f8fafc', outline: 'none' }} />
          </div>
        )}
        ...
        <div style={{ background: 'white', borderRadius: 18, padding: '18px 16px', border: '0.5px solid #e2e8f0' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <MethodBtn active={paymentMethod === 'on_delivery'} onClick={() => setPaymentMethod('on_delivery')}
              icon={<CreditCard size={22} color={paymentMethod === 'on_delivery' ? '#16a34a' : '#94a3b8'} />} label="On Delivery" sub="Pay the driver" />
            <MethodBtn active={paymentMethod === 'on_pickup'} onClick={() => setPaymentMethod('on_pickup')}
              icon={<Smartphone size={22} color={paymentMethod === 'on_pickup' ? '#16a34a' : '#94a3b8'} />} label="On Pickup" sub="Pay at stall" />
          </div>
        </div>
        ...
        <div style={{ background: '#0f172a', borderRadius: 18, padding: '18px 16px' }}>
          ...
          {cartItems.map(item => (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 12, color: 'white', fontWeight: 600 }}>M{((item.menu_items?.price || 0) * item.quantity).toFixed(2)}</span>
            </div>
          ))}
        ...
        {error && (
          <div style={{ background: '#fef2f2', color: '#dc2626', padding: '12px 14px', borderRadius: 12, fontSize: 13 }}>{error}</div>
        )}

        <button onClick={handlePlaceOrder} disabled={loading}
          style={{ width: '100%', background: loading ? '#86efac' : '#16a34a', color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer' }}>
